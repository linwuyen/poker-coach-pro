...
function TrainPage({ profile, history, relevant, onStart }: { profile: PlayerProfile; history: HistoryItem[]; relevant: Scenario[]; onStart: (title: string, scenarios: Scenario[]) => void }) {
  ...
  return <div><div className="mb-6 max-w-2xl"><h2 className="text-2xl font-semibold">選擇訓練模式</h2><p className="mt-2 text-sm text-slate-400">預設使用系統推薦。專項訓練仍會記錄信心、掌握度與延遲留存。</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => { window.location.hash = 'contrastive-trainer'; }} className="rounded-xl border border-fuchsia-500/30 bg-fuchsia-500/8 px-3 py-2 text-xs text-fuchsia-200">Contrastive</button><button type="button" onClick={() => { window.location.hash = 'boundary-map'; }} className="rounded-xl border border-violet-500/30 bg-violet-500/8 px-3 py-2 text-xs text-violet-200">Boundary Map</button><button type="button" onClick={() => { window.location.hash = 'solver-benchmark'; }} className="rounded-xl border border-amber-500/30 bg-amber-500/8 px-3 py-2 text-xs text-amber-200">Solver Holdout</button></div></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{modes.map(mode => <ModeCard key={mode.title} {...mode} onClick={() => onStart(mode.title, mode.items)} />)}</div></div>;
}
