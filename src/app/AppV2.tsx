...
interface ActiveSession { title: string; scenarios: Scenario[]; candidatePool: Scenario[]; }
...
export default function AppV2() {
  ...
  const startSession = (title: string, selected: Scenario[], pool: Scenario[] = selected) => {
    const unique = [...new Map(selected.map(scenario => [scenario.id, scenario])).values()];
    const uniquePool = [...new Map(pool.map(scenario => [scenario.id, scenario])).values()];
    if (!unique.length) { setNotice('目前沒有符合條件的題目。若是複習，系統不會為了刷題而提前破壞間隔。'); return; }
    setNotice(null);
    setActiveSession({ title, scenarios: unique, candidatePool: uniquePool.length ? uniquePool : unique });
    setPage('train');
  ...
  return (
    <AppShell page={page} onPageChange={changePage} onOpenSettings={() => setSettingsOpen(true)}>
      ...
      {activeSession ? <TrainingSession title={activeSession.title} scenarios={activeSession.scenarios} candidatePool={activeSession.candidatePool} history={history} onRecord={recordHistory} onExit={() => setActiveSession(null)} onComplete={() => { setActiveSession(null); setPage('today'); }} /> : <>
        {page === 'today' && <TodayPage dailyPlan={dailyPlan} dueCount={dueScenarios.length} weaknesses={weaknesses} metrics={metrics} weekItems={weekItems} onStart={() => startSession('今日自動教練', dailyPlan.items.map(item => item.scenario), relevantScenarios)} onReview={() => startSession('到期提取複習', dueScenarios, dueScenarios)} onNavigate={changePage} />}
...
function TrainPage({ profile, history, relevant, onStart }: {
  profile: PlayerProfile;
  history: HistoryItem[];
  relevant: Scenario[];
  onStart: (title: string, scenarios: Scenario[], pool?: Scenario[]) => void;
}) {
  ...
  const tracks = [
    { title: '自動教練', description: '系統持續依你的最新結果改下一手，不用選工具', icon: <Sparkles className="h-6 w-6" />, items: daily, pool: relevant, featured: true },
    { title: '翻前', description: 'RFI、BB defense、3-Bet、4-Bet 與短碼；答完直接顯示可驗證 strategy', icon: <Layers3 className="h-6 w-6" />, items: preflop.slice(0, 24), pool: preflop },
    { title: '翻後', description: 'Flop / Turn / River；答完原地看 Range、EV、Boundary 與漏點', icon: <Brain className="h-6 w-6" />, items: postflop.slice(0, 24), pool: postflop },
    { title: '錦標賽', description: '短碼、ICM、PKO、Satellite；只顯示可信 $EV / priority 證據', icon: <Target className="h-6 w-6" />, items: tournament.slice(0, 24), pool: tournament },
  ];
  return <div className="space-y-6"><div className="max-w-3xl"><div className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-400">一條訓練流程</div><h2 className="mt-2 text-2xl font-semibold">只選你要練的方向</h2><p className="mt-2 text-sm leading-6 text-slate-400">進場後只做：看牌 → 決策 → 深挖 → 下一手。模擬牌局已知的資料與你的選擇全部自動帶入分析，不再叫你開 Range / Equity / Solver 後重填一遍。</p></div><div className="grid gap-4 md:grid-cols-2">{tracks.map(track => <ModeCard key={track.title} {...track} onClick={() => onStart(track.title, track.items, track.pool)} />)}</div><div className="rounded-2xl border border-slate-800 bg-slate-900/45 p-5 text-sm leading-6 text-slate-400"><span className="font-semibold text-slate-200">進階 workbench 沒有刪除。</span> 它們保留給獨立研究、benchmark 與診斷；正常訓練不需要先理解這些工具，也不再從這裡分流。</div></div>;
}
...
function ProgressPage({ history, metrics, weaknesses, mastery, dueScenarios, onStart, onRecord, onExport, onImport }: {
  ...
  onStart: (title: string, scenarios: Scenario[], pool?: Scenario[]) => void; onRecord: (item: HistoryItem) => void; onExport: () => void; onImport: () => void;
}) {
