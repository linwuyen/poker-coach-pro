...
import {
  ArrowRight, BarChart3, Brain, Calculator, CalendarClock, CheckCircle2, Clock3, Database,
  Download, EyeOff, GitCompareArrows, Layers3, Network, Play, RotateCcw, Ruler, Scale,
...
function TrainPage({ profile, history, relevant, onStart }: { profile: PlayerProfile; history: HistoryItem[]; relevant: Scenario[]; onStart: (title: string, scenarios: Scenario[]) => void }) {
  ...
  const tracks = [
    { title: '自動教練', description: '不用選主題，系統直接挑現在最值錢的題', icon: <Sparkles className="h-6 w-6" />, items: daily, featured: true },
    { title: '翻前', description: 'RFI、BB defense、3-Bet、4-Bet 與短碼', icon: <Layers3 className="h-6 w-6" />, items: relevant.filter(s => s.steps.some(step => step.street === 'Preflop')).slice(0, 24) },
    { title: '翻後', description: 'Flop / Turn / River 決策、尺寸與 bluff catch', icon: <Brain className="h-6 w-6" />, items: relevant.filter(s => s.steps.some(step => step.street !== 'Preflop')).slice(0, 24) },
    { title: '錦標賽', description: '短碼、ICM、PKO、Satellite；不把 chip-EV 當 $EV', icon: <Target className="h-6 w-6" />, items: relevant.filter(s => s.type === 'Tournament').slice(0, 24) },
  ];
  return <div className="space-y-6"><div className="max-w-3xl"><h2 className="text-2xl font-semibold">先選訓練方向，不要先選工具</h2><p className="mt-2 text-sm leading-6 text-slate-400">90% 的時間只需要下面四個入口。Solver、Range、Equity、Boundary、ICM 是解題工具，收在同一個「工具箱」裡，需要時再開。</p></div><div className="grid gap-4 md:grid-cols-2">{tracks.map(track => <ModeCard key={track.title} {...track} onClick={() => onStart(track.title, track.items)} />)}</div><Toolbox /><details className="rounded-2xl border border-slate-800 bg-slate-900/45"><summary className="cursor-pointer list-none px-5 py-4 text-sm font-semibold text-slate-200">策略查詢器 <span className="ml-2 text-xs font-normal text-slate-500">需要查特定 hand / profile 時再展開</span></summary><div className="border-t border-slate-800 p-4 md:p-6"><StrategyExplorer /></div></details></div>;
}

function Toolbox() {
  const groups = [
    { title: 'Solver / Range', tools: [{ label: 'Solver Curriculum', hash: 'solver-corpus', icon: <Database className="h-4 w-4" /> }, { label: 'Strategy Surface', hash: 'strategy-surface', icon: <BarChart3 className="h-4 w-4" /> }, { label: 'Range Reading', hash: 'range-reading', icon: <Brain className="h-4 w-4" /> }] },
    { title: '數學 / Boundary', tools: [{ label: 'Decision Boundary', hash: 'boundary-map', icon: <Ruler className="h-4 w-4" /> }, { label: 'Equity', hash: 'equity-workbench', icon: <Calculator className="h-4 w-4" /> }, { label: 'Sizing', hash: 'sizing-trainer', icon: <Target className="h-4 w-4" /> }] },
    { title: 'MTT / Transfer', tools: [{ label: 'ICM / $EV', hash: 'icm-workbench', icon: <Scale className="h-4 w-4" /> }, { label: 'Contrastive', hash: 'contrastive-trainer', icon: <GitCompareArrows className="h-4 w-4" /> }, { label: 'Hidden Holdout', hash: 'solver-benchmark', icon: <EyeOff className="h-4 w-4" /> }] },
  ];
  return <Panel title="工具箱" subtitle="工具不再各自佔一個頁面；先知道你要練什麼，再選工具"><div className="grid gap-4 lg:grid-cols-3">{groups.map(group => <div key={group.title} className="rounded-xl border border-slate-800 bg-slate-950/35 p-4"><div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">{group.title}</div><div className="mt-3 flex flex-wrap gap-2">{group.tools.map(tool => <ToolLink key={tool.hash} {...tool} />)}</div></div>)}</div></Panel>;
}
