...
import { BarChart3, Brain, CalendarCheck2, ChartNoAxesCombined, Database, FileUp, FlaskConical, GitCompareArrows, Layers3, Settings2, ShieldCheck, Shuffle, TreePine, Trophy } from 'lucide-react';
  ...
  return (
    <div className="relative min-h-screen bg-slate-950 text-slate-100">
      ...
      <div className="relative z-10 min-h-screen pb-24 lg:ml-64 lg:pb-0">
        <header className="sticky top-0 z-20 border-b border-slate-800/80 bg-slate-950/78 px-4 py-3 backdrop-blur-xl md:px-8 lg:px-10"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3"><div><h1 className="text-lg font-semibold tracking-tight md:text-xl">{current.label}</h1><p className="text-xs text-slate-500">{current.description}</p></div><div className="flex items-center gap-2"><button type="button" onClick={() => openLab('hand-history')} className="hidden rounded-lg border border-emerald-500/20 px-3 py-2 text-xs font-medium text-emerald-300 hover:bg-emerald-500/8 md:block">匯入 HH</button><button type="button" onClick={() => openLab('postflop-truth')} className="hidden rounded-lg border border-cyan-500/20 px-3 py-2 text-xs font-medium text-cyan-300 hover:bg-cyan-500/8 lg:block">Postflop v3</button><button type="button" onClick={() => openLab('truth-ops')} className="hidden rounded-lg border border-blue-500/20 px-3 py-2 text-xs font-medium text-blue-300 hover:bg-blue-500/8 xl:block">Truth Ops</button><button type="button" onClick={onOpenSettings} className="pc-interactive rounded-lg border border-slate-800 px-3 py-2 text-xs font-medium text-slate-400 hover:bg-slate-900 hover:text-slate-100 lg:hidden">設定</button></div></div></header>
