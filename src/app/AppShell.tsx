...
import { BarChart3, Brain, CalendarCheck2, Database, GitCompareArrows, Settings2, Shuffle } from 'lucide-react';
  ...
  return (
    <div className="relative min-h-screen bg-slate-950 text-slate-100">
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-slate-800/80 bg-slate-950/92 px-4 py-5 backdrop-blur-xl lg:flex lg:flex-col">
        ...
        <div className="mt-5 rounded-xl border border-emerald-500/15 bg-emerald-500/5 p-3 text-xs leading-5 text-slate-500"><span className="font-semibold text-slate-300">唯一主迴路</span><br />看牌 → 自己決策 → 一句話修正 → 真正邊界/transfer → 自適應下一手。換皮題共享 canonical mastery，不再膨脹掌握度。</div>
      ...
      <div className="relative z-10 min-h-screen pb-24 lg:ml-64 lg:pb-0">
        <header className="sticky top-0 z-20 border-b border-slate-800/80 bg-slate-950/78 px-4 py-3 backdrop-blur-xl md:px-8 lg:px-10"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3"><div><h1 className="text-lg font-semibold tracking-tight md:text-xl">{current.label}</h1><p className="text-xs text-slate-500">{current.description}</p></div><div className="flex items-center gap-2"><button type="button" onClick={() => openLab('semantic-counterfactual')} className="hidden rounded-lg border border-cyan-500/20 px-3 py-2 text-xs font-medium text-cyan-300 hover:bg-cyan-500/8 md:block">語義反事實</button><button type="button" onClick={() => openLab('variant-trainer')} className="hidden rounded-lg border border-violet-500/20 px-3 py-2 text-xs font-medium text-violet-300 hover:bg-violet-500/8 xl:block">泛化變式</button><button type="button" onClick={() => openLab('solver-corpus')} className="hidden rounded-lg border border-blue-500/20 px-3 py-2 text-xs font-medium text-blue-300 hover:bg-blue-500/8 md:block">Solver 題庫</button><button type="button" onClick={onOpenSettings} className="pc-interactive rounded-lg border border-slate-800 px-3 py-2 text-xs font-medium text-slate-400 hover:bg-slate-900 hover:text-slate-100 lg:hidden">設定</button></div></div></header>
