...
function openTrainingAssistant() {
  ...
  window.open(`${base}#companion`, 'poker-coach-training-assistant', 'popup=yes,width=480,height=920,resizable=yes,scrollbars=yes');
}
    ...
    <div className="relative min-h-screen bg-slate-950 text-slate-100">
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-slate-800/80 bg-slate-950/92 px-4 py-5 backdrop-blur-xl lg:flex lg:flex-col">
        <button type="button" onClick={() => onPageChange('today')} className="pc-interactive flex items-center gap-3 rounded-2xl px-2 py-2 text-left">
          ...
          <div><div className="font-semibold tracking-tight">想高龍了 德撲訓練機</div><div className="text-xs text-slate-500">Training-first Architecture</div></div>
        </button>
        ...
        <button type="button" onClick={openTrainingAssistant} className="pc-interactive mt-5 flex w-full items-center gap-3 rounded-xl border border-emerald-500/25 bg-emerald-500/6 px-3 py-3 text-left text-emerald-200 hover:bg-emerald-500/10"><MonitorUp className="h-5 w-5" /><span><span className="block text-sm font-semibold">開啟訓練助手</span><span className="block text-[11px] text-emerald-300/55">同步目前訓練題 · 作答後展開分析</span></span></button>
        <div className="mt-5 rounded-xl border border-slate-800 bg-slate-900/40 p-3 text-xs leading-5 text-slate-500"><span className="font-semibold text-slate-300">訓練主迴路</span><br />TrainingSession → HandStateBus → Retrieval Gate → Feedback → Strategy / Best Intervention。作答前不偷看答案，作答後才展開診斷。</div>
        <div className="mt-auto space-y-2"><button type="button" onClick={onOpenSettings} className="pc-interactive flex w-full items-center gap-3 rounded-xl border border-slate-800 px-3 py-3 text-left text-slate-400 hover:border-slate-700 hover:bg-slate-900 hover:text-slate-100"><Settings2 className="h-5 w-5" /><span><span className="block text-sm font-semibold">玩家與資料設定</span><span className="block text-[11px] text-slate-500">個人化、備份、加密同步</span></span></button><p className="px-2 text-[11px] leading-relaxed text-slate-600">工具只服務訓練決策：系統先選題，作答後再打開最相關的分析工具。</p></div>
      </aside>

      <div className="relative z-10 min-h-screen pb-24 lg:ml-64 lg:pb-0">
        <header className="sticky top-0 z-20 border-b border-slate-800/80 bg-slate-950/78 px-4 py-3 backdrop-blur-xl md:px-8 lg:px-10"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3"><div><h1 className="text-lg font-semibold tracking-tight md:text-xl">{current.label}</h1><p className="text-xs text-slate-500">{current.description}</p></div><div className="flex gap-2"><button type="button" onClick={openTrainingAssistant} className="pc-interactive rounded-lg border border-emerald-500/25 px-3 py-2 text-xs font-medium text-emerald-300 hover:bg-emerald-500/8 lg:hidden">訓練助手</button><button type="button" onClick={onOpenSettings} className="pc-interactive rounded-lg border border-slate-800 px-3 py-2 text-xs font-medium text-slate-400 hover:bg-slate-900 hover:text-slate-100 lg:hidden">設定</button></div></div></header>
