...
import { BarChart3, Brain, CalendarCheck2, ChartNoAxesCombined, Database, FileUp, GitCompareArrows, Settings2, Shuffle, TreePine } from 'lucide-react';
  ...
  return (
    <div className="relative min-h-screen bg-slate-950 text-slate-100">
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-slate-800/80 bg-slate-950/92 px-4 py-5 backdrop-blur-xl lg:flex lg:flex-col">
        ...
        <div className="mt-4 rounded-xl border border-emerald-500/15 bg-emerald-500/5 p-3 text-xs leading-5 text-slate-500"><span className="font-semibold text-slate-300">閉環</span><br />實戰 exposure → 找漏點 → 教學/solver transfer → 延遲驗證 → 回看實戰。沒有真實 solver / population evidence 就不製造假精度。</div>
      ...
      <div className="relative z-10 min-h-screen pb-24 lg:ml-64 lg:pb-0">
        <header className="sticky top-0 z-20 border-b border-slate-800/80 bg-slate-950/78 px-4 py-3 backdrop-blur-xl md:px-8 lg:px-10"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3"><div><h1 className="text-lg font-semibold tracking-tight md:text-xl">{current.label}</h1><p className="text-xs text-slate-500">{current.description}</p></div><div className="flex items-center gap-2"><button type="button" onClick={() => openLab('hand-history')} className="hidden rounded-lg border border-emerald-500/20 px-3 py-2 text-xs font-medium text-emerald-300 hover:bg-emerald-500/8 md:block">匯入 HH</button><button type="button" onClick={() => openLab('effectiveness')} className="hidden rounded-lg border border-amber-500/20 px-3 py-2 text-xs font-medium text-amber-300 hover:bg-amber-500/8 md:block">學習成效</button><button type="button" onClick={() => openLab('solver-corpus')} className="hidden rounded-lg border border-blue-500/20 px-3 py-2 text-xs font-medium text-blue-300 hover:bg-blue-500/8 xl:block">Solver 題庫</button><button type="button" onClick={onOpenSettings} className="pc-interactive rounded-lg border border-slate-800 px-3 py-2 text-xs font-medium text-slate-400 hover:bg-slate-900 hover:text-slate-100 lg:hidden">設定</button></div></div></header>
