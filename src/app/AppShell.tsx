...
import { BarChart3, Brain, CalendarCheck2, Database, FileUp, GitCompareArrows, Settings2, Shuffle } from 'lucide-react';
    ...
    <div className="relative min-h-screen bg-slate-950 text-slate-100">
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-slate-800/80 bg-slate-950/92 px-4 py-5 backdrop-blur-xl lg:flex lg:flex-col">
        ...
        <div className="mt-4 space-y-2 border-t border-slate-800/70 pt-4">
          <button type="button" onClick={() => openLab('hand-history')} className="pc-interactive flex w-full items-center gap-3 rounded-xl border border-emerald-500/15 bg-emerald-500/5 px-3 py-3 text-left text-emerald-200 hover:border-emerald-500/35"><FileUp className="h-5 w-5" /><span><span className="block text-sm font-semibold">實戰 HH 匯入</span><span className="block text-[11px] text-slate-500">PokerStars / GGPoker → leak evidence</span></span></button>
          <button type="button" onClick={() => openLab('semantic-counterfactual')} className="pc-interactive flex w-full items-center gap-3 rounded-xl border border-cyan-500/15 bg-cyan-500/5 px-3 py-3 text-left text-cyan-200 hover:border-cyan-500/35"><GitCompareArrows className="h-5 w-5" /><span><span className="block text-sm font-semibold">語義反事實</span><span className="block text-[11px] text-slate-500">One-variable solver label flips</span></span></button>
          <button type="button" onClick={() => openLab('variant-trainer')} className="pc-interactive flex w-full items-center gap-3 rounded-xl border border-violet-500/15 bg-violet-500/5 px-3 py-3 text-left text-violet-200 hover:border-violet-500/35"><Shuffle className="h-5 w-5" /><span><span className="block text-sm font-semibold">泛化變式</span><span className="block text-[11px] text-slate-500">安全同構 transfer nodes</span></span></button>
          <button type="button" onClick={() => openLab('solver-corpus')} className="pc-interactive flex w-full items-center gap-3 rounded-xl border border-blue-500/15 bg-blue-500/5 px-3 py-3 text-left text-blue-200 hover:border-blue-500/35"><Database className="h-5 w-5" /><span><span className="block text-sm font-semibold">Solver 題庫</span><span className="block text-[11px] text-slate-500">PokerBench 1k + 10k verified rows</span></span></button>
        </div>
        <div className="mt-5 rounded-xl border border-emerald-500/15 bg-emerald-500/5 p-3 text-xs leading-5 text-slate-500"><span className="font-semibold text-slate-300">閉環</span><br />實戰 exposure → 找漏點 → 教學/solver transfer → 延遲驗證 → 回看實戰。沒有真實 solver / population evidence 就不製造假精度。</div>
      ...
      <div className="relative z-10 min-h-screen pb-24 lg:ml-64 lg:pb-0">
        <header className="sticky top-0 z-20 border-b border-slate-800/80 bg-slate-950/78 px-4 py-3 backdrop-blur-xl md:px-8 lg:px-10"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3"><div><h1 className="text-lg font-semibold tracking-tight md:text-xl">{current.label}</h1><p className="text-xs text-slate-500">{current.description}</p></div><div className="flex items-center gap-2"><button type="button" onClick={() => openLab('hand-history')} className="hidden rounded-lg border border-emerald-500/20 px-3 py-2 text-xs font-medium text-emerald-300 hover:bg-emerald-500/8 md:block">匯入 HH</button><button type="button" onClick={() => openLab('semantic-counterfactual')} className="hidden rounded-lg border border-cyan-500/20 px-3 py-2 text-xs font-medium text-cyan-300 hover:bg-cyan-500/8 xl:block">語義反事實</button><button type="button" onClick={() => openLab('solver-corpus')} className="hidden rounded-lg border border-blue-500/20 px-3 py-2 text-xs font-medium text-blue-300 hover:bg-blue-500/8 md:block">Solver 題庫</button><button type="button" onClick={onOpenSettings} className="pc-interactive rounded-lg border border-slate-800 px-3 py-2 text-xs font-medium text-slate-400 hover:bg-slate-900 hover:text-slate-100 lg:hidden">設定</button></div></div></header>
