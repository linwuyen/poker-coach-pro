...
import { Feedback, ActionType, Scenario, ScenarioStep, Card, Suit, Rank } from './types';
import { 
...
function reskinScenario(scenario: Scenario): Scenario {
...
function matchesSearch(s: Scenario, query: string): boolean {
...
const NINE_UTG1_COMBOS = new Set([
  ...Array.from(NINE_UTG_COMBOS),
  '66', 'K9s', 'Q9s', 'KQo'
]);

const NINE_UTG2_COMBOS = new Set([
  ...Array.from(NINE_UTG1_COMBOS),
  '55', 'J9s', 'T8s', '98s', 'ATo', 'KJo'
]);

const NINE_MP_COMBOS = new Set([
  ...Array.from(NINE_UTG2_COMBOS),
  '44', '33', '22', 'K8s', 'Q8s', 'J8s', '87s', 'QJo'
]);

const NINE_HJ_COMBOS = new Set([
  ...Array.from(NINE_MP_COMBOS),
  'K7s', 'K6s', 'K5s', 'J7s', 'T7s', '97s', '86s', '76s', 'KTo', 'QTo'
]);

function isComboInGtoRange(combo: string, position: string, tableSize: '6max' | '9max'): boolean {
...
const isPositionMatch = (posA: string, posB: string, tableSize: '6max' | '9max' = '9max'): boolean => {
...
const MiniCard: React.FC<{ card?: Card; hidden?: boolean }> = ({ card, hidden }) => {
  if (hidden || !card) {
    return (
