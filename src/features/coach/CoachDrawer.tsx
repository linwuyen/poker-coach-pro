...
export function CoachDrawer({ open, scenario, step, feedback, selectedAction, onClose }: CoachDrawerProps) {
  ...
  const ask = async () => {
    ...
    try {
      if (mode === 'offline') {
        const response = generateOfflineFollowUp(context, step, question);
    ...
    <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm" onMouseDown={event => { if (event.currentTarget === event.target) onClose(); }}>
      <aside className="ml-auto flex h-full w-full max-w-xl flex-col border-l border-slate-800 bg-slate-950 text-slate-100 shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          <div className="whitespace-pre-wrap text-sm leading-7 text-slate-300">{analysis}</div>
          {error && <div className="mt-4 rounded-xl border border-red-500/25 bg-red-500/8 p-3 text-sm text-red-300">{error} GitHub Pages 沒有後端時請切回離線教練。</div>}
        </div>
        <footer className="border-t border-slate-800 p-4">
          <div className="flex gap-2">
            <textarea value={question} onChange={event => setQuestion(event.target.value)} rows={2} placeholder="例如：哪些 turn 會讓答案反轉？" className="min-h-12 flex-1 resize-none rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm outline-none focus:border-emerald-500" />
            <button type="button" onClick={ask} disabled={loading || !question.trim()} className="grid w-12 place-items-center rounded-xl bg-emerald-500 text-emerald-950 disabled:opacity-40">{loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <Send className="h-5 w-5" />}</button>
          </div>
        </footer>
      </aside>
    </div>
