...
export function EffectivenessDashboard({ onExit }: { onExit: () => void }) {
  ...
  return <div className="min-h-screen bg-slate-950 px-4 py-6 text-slate-100 md:px-8"><div className="mx-auto max-w-6xl">
    ...
    <section className="pc-hero-glow mt-6 rounded-3xl border border-amber-500/20 bg-[linear-gradient(135deg,rgba(245,158,11,0.12),rgba(15,23,42,0.8))] p-6 md:p-8"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-amber-300"><ChartNoAxesCombined className="h-4 w-4" />P5-C · Learning Effectiveness</div><h1 className="mt-3 text-3xl font-bold">Before → Training → Follow-up：看你有沒有真的留下來</h1><p className="mt-3 max-w-4xl text-sm leading-7 text-slate-300">固定比較未洩漏 holdout、transfer、delayed retention 與已驗證的 real-game leak。這是單一玩家的 observational before/after，不是 RCT，因此只報趨勢與 evidence level，不把時間相關性寫成因果。</p><div className="mt-4 inline-flex rounded-xl border border-amber-500/20 bg-amber-500/7 px-4 py-2 text-sm font-semibold text-amber-200">Evidence：{report.evidenceLevel.toUpperCase()}</div></section>
    ...
    <section className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4"><DeltaCard label="Holdout accuracy" delta={report.holdout} suffix="%" /><DeltaCard label="Transfer accuracy" delta={report.transfer} suffix="%" /><DeltaCard label="Delayed retention" delta={report.delayedRetention} suffix="%" /><DeltaCard label="Verified real-game leak" delta={report.realGameLeak} suffix=" BB/100" /></section>
...
function WindowCard({ title, metrics }: { title: string; metrics: ReturnType<typeof evaluateLearningEffectiveness>['windows'][number] }) {
  return <div className="rounded-2xl border border-slate-800 bg-slate-900/55 p-5"><h2 className="font-semibold">{title}</h2><div className="mt-4 space-y-2 text-xs"><Row label="Training decisions" value={metrics.trainingDecisions} /><Row label="Holdout" value={`${metrics.holdoutAttempts} · ${fmt(metrics.holdoutAccuracy, '%')}`} /><Row label="Transfer" value={`${metrics.transferAttempts} · ${fmt(metrics.transferAccuracy, '%')}`} /><Row label="Delayed" value={`${metrics.delayedAttempts} · ${fmt(metrics.delayedRetention, '%')}`} /><Row label="Real hands" value={metrics.realGameHands} /><Row label="Verified leak obs" value={metrics.verifiedLeakObservations} /></div></div>;
}
