...
export function SettingsDrawer({ open, profile, history, starredIds, onClose, onEditProfile, onRestore }: SettingsDrawerProps) {
  ...
  return <div className="fixed inset-0 z-[115] bg-black/60 backdrop-blur-sm" onMouseDown={event => { if (event.currentTarget === event.target) onClose(); }}>
    <aside className="ml-auto h-full w-full max-w-lg overflow-y-auto border-l border-slate-800 bg-slate-950 p-5 text-slate-100 shadow-2xl">
      <header className="flex items-center justify-between"><div className="flex items-center gap-3"><Settings2 className="h-5 w-5 text-emerald-400" /><div><div className="font-semibold">設定</div><div className="text-xs text-slate-500">正常訓練不需要填任何額外資料</div></div></div><button type="button" onClick={onClose} className="rounded-lg p-2 text-slate-500 hover:bg-slate-800"><X className="h-5 w-5" /></button></header>

      <section className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/55 p-5"><div className="font-semibold">玩家偏好</div><p className="mt-2 text-xs leading-6 text-slate-500">這些偏好只用來決定你比較常看到哪些牌局。訓練中不會再要求重填。</p><div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-400"><Value label="賽制" value={profile.formats.join(' / ')} /><Value label="桌型" value={profile.tableSizes.join(' / ')} /><Value label="籌碼" value={profile.stackBands.join(' / ')} /><Value label="每日基準" value={`${profile.dailyQuestions} 決策`} /></div><button type="button" onClick={onEditProfile} className="mt-4 w-full rounded-xl border border-emerald-500/30 px-4 py-3 text-sm font-semibold text-emerald-300">修改偏好</button></section>

      <details className="mt-5 rounded-2xl border border-slate-800 bg-slate-900/40 p-5"><summary className="cursor-pointer list-none font-semibold text-slate-300"><span className="flex items-center gap-2"><Cloud className="h-4 w-4 text-blue-400" />進階：跨裝置加密同步</span></summary><p className="mt-3 text-xs leading-relaxed text-slate-500">完全選配。只有你要使用自己的 HTTPS PUT/GET endpoint 時才需要設定；日常訓練不需要。</p>
