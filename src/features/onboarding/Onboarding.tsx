  ...
  return (
    <div className="fixed inset-0 z-[120] overflow-y-auto bg-slate-950/95 p-4 text-slate-100 backdrop-blur">
      <div className="mx-auto my-6 max-w-3xl rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-2xl md:p-9">
        ...
        <Section title="主要賽制"><Choice active={profile.formats.includes('cash')} label="Cash Game" onClick={() => setProfile(value => ({ ...value, formats: toggle(value.formats, 'cash') }))} /><Choice active={profile.formats.includes('tournament')} label="Tournament" onClick={() => setProfile(value => ({ ...value, formats: toggle(value.formats, 'tournament') }))} /></Section>
        <Section title="常玩桌型"><Choice active={profile.tableSizes.includes('6max')} label="6-Max" onClick={() => setProfile(value => ({ ...value, tableSizes: toggle(value.tableSizes, '6max') }))} /><Choice active={profile.tableSizes.includes('9max')} label="9-Max" onClick={() => setProfile(value => ({ ...value, tableSizes: toggle(value.tableSizes, '9max') }))} /></Section>
        <Section title="常見有效籌碼">{STACKS.map(item => <Choice key={item.id} active={profile.stackBands.includes(item.id)} label={item.label} onClick={() => setProfile(value => ({ ...value, stackBands: toggle(value.stackBands, item.id) }))} />)}</Section>
        <Section title="目前程度">{([['beginner', '新手'], ['intermediate', '有基礎'], ['advanced', '進階']] as Array<[PlayerExperience, string]>).map(([id, label]) => <Choice key={id} active={profile.experience === id} label={label} onClick={() => setProfile(value => ({ ...value, experience: id }))} />)}</Section>
        <Section title="最想改善">{FOCUS.map(item => <Choice key={item.id} active={profile.focusAreas.includes(item.id)} label={item.label} onClick={() => setProfile(value => ({ ...value, focusAreas: toggle(value.focusAreas, item.id) }))} />)}</Section>
        <Section title="每日題數">{([8, 12, 20] as const).map(count => <Choice key={count} active={profile.dailyQuestions === count} label={`${count} 題`} onClick={() => setProfile(value => ({ ...value, dailyQuestions: count }))} />)}</Section>
...
function Section({ title, children }: { title: string; children: ReactNode }) { return <section className="mt-7"><h2 className="mb-3 text-sm font-semibold text-slate-300">{title}</h2><div className="flex flex-wrap gap-2">{children}</div></section>; }
function Choice({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) { return <button type="button" onClick={onClick} className={`rounded-xl border px-4 py-2.5 text-sm font-semibold transition ${active ? 'border-emerald-500/50 bg-emerald-500/12 text-emerald-300' : 'border-slate-700 bg-slate-950/50 text-slate-400 hover:text-white'}`}>{label}</button>; }
