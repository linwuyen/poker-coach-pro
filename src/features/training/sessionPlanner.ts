...
export interface PlannedScenario { scenario: Scenario; reason: TrainingReason; }
...
function stableMixScore(id: string, now: number): number { const day = Math.floor(now / 86400000); return [...`${id}-${day}`].reduce((hash, character) => ((hash * 31) + character.charCodeAt(0)) >>> 0, 7); }
...
export function buildDailyTrainingPlan(scenarios: Scenario[], history: HistoryItem[], size = 12, now = Date.now(), profile?: PlayerProfile): DailyTrainingPlan {
  ...
  const order = (a: Scenario, b: Scenario): number => {
    const relevance = profile ? scenarioProfileScore(b, profile) - scenarioProfileScore(a, profile) : 0;
    return relevance || stableMixScore(a.id, now) - stableMixScore(b.id, now);
  };
  const take = (candidates: Scenario[], limit: number, reason: TrainingReason) => {
    [...candidates].sort(order).forEach(scenario => {
      if (items.length >= size || counts[reason] >= limit || selected.has(scenario.id)) return;
      selected.add(scenario.id); items.push({ scenario, reason }); counts[reason] += 1;
    });
  };
  ...
  const recentMistakeIds = history.filter(item => item.trainingType !== 'custom' && !isHistoryCorrect(item)).sort((a, b) => b.timestamp - a.timestamp).map(item => item.scenarioId);
  take(recentMistakeIds.map(id => byId.get(id)).filter((scenario): scenario is Scenario => Boolean(scenario && relevantIds.has(scenario.id))), limits.mistake, 'recent-mistake');

  const unseen = relevant.filter(scenario => !latestScenario.has(scenario.id));
  take(unseen, limits.fresh, 'new');
  const benchmark = relevant.filter(scenario => !latestScenario.has(scenario.id) && !selected.has(scenario.id)).filter(scenario => !scenario.category?.some(category => weakCategories.includes(category))).sort((a, b) => stableMixScore(a.id, now + 7919) - stableMixScore(b.id, now + 7919));
  take(benchmark, limits.benchmark, 'benchmark');
  take([...relevant].sort((a, b) => stableMixScore(a.id, now) - stableMixScore(b.id, now)), size, 'mixed');
...
export function getDueScenarioIds(history: HistoryItem[], now = Date.now()): string[] {
  const due = [...latestByMasteryKey(history).values()].filter(item => isDue(item, now)).sort((a, b) => (a.nextReviewAt || Infinity) - (b.nextReviewAt || Infinity));
