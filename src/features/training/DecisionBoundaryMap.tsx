...
export function DecisionBoundaryMap({ onExit }: { onExit: () => void }) {
  ...
  const [bluffShare, setBluffShare] = useState(30);
  ...
  return <div className="min-h-screen bg-slate-950 px-4 py-6 text-slate-100 md:px-8"><div className="mx-auto max-w-6xl">
    ...
    <section className="mt-6 grid gap-4 md:grid-cols-3"><Metric label="Equity boundary" value={`≈ ${equityBoundary}% pot`} detail="超過此下注尺寸後，目前 equity 會跨向 Fold" /><Metric label="Bluff boundary" value={`≈ ${bluffBoundary}% pot`} detail="極化 river 模型下的 bluff-catch 邊界" /><Metric label="Truth" value="exact-math" detail="B / (P + 2B)" /></section>
