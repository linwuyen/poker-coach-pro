...
type LockedAnswer = { choice: string; confidence: ConfidenceLevel; durationMs: number };
...
export function SemanticCounterfactualSession({
  pairs,
  history,
  onRecord,
  onExit,
  ...
  if (!pair || !row) {
    return (
      <div className="mx-auto max-w-3xl rounded-3xl border border-emerald-500/20 bg-emerald-500/6 p-8 text-center text-slate-100">
        <CheckCircle2 className="mx-auto h-10 w-10 text-emerald-400" />
        <h2 className="mt-4 text-2xl font-bold">今日語義 transfer 完成</h2>
        <p className="mt-2 text-sm text-slate-400">你完成了 {pairs.length} 組、{pairs.length * 2} 個 solver-backed 決策。</p>
        <div className="mt-6 flex justify-center gap-3">
          <button onClick={onComplete} className="rounded-xl bg-emerald-500 px-5 py-3 font-semibold text-emerald-950">完成</button>
          <button onClick={onExit} className="rounded-xl border border-slate-700 px-5 py-3 text-slate-300">離開</button>
        </div>
      </div>
    );
  }
  ...
  const lockDecision = () => {
    ...
    const locked: LockedAnswer = { choice, confidence, durationMs: Date.now() - startedAt.current };
    ...
    <div className="mx-auto max-w-5xl space-y-5 text-slate-100">
      <header className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
        <div className="flex flex-wrap items-center gap-3">
          <button onClick={onExit} className="flex items-center gap-2 rounded-lg px-2 py-2 text-sm text-slate-400 hover:bg-slate-800">
            <ArrowLeft className="h-4 w-4" />離開
          </button>
          <div className="flex-1">
            <div className="flex justify-between text-sm"><b>{title}</b><span className="font-mono text-slate-500">PAIR {index + 1}/{pairs.length}</span></div>
            <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-800"><div className="h-full bg-cyan-400" style={{ width: `${progress}%` }} /></div>
          </div>
        </div>
      </header>
      ...
      {!revealed ? (
        <>
          <section className="rounded-3xl border border-cyan-500/20 bg-[linear-gradient(135deg,rgba(6,182,212,0.12),rgba(15,23,42,0.82))] p-6 md:p-8">
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-cyan-300"><ShieldCheck className="h-4 w-4" />verified-solver · {side === 'left' ? 'A' : 'B'}</div>
            <h2 className="mt-3 text-2xl font-bold">{side === 'left' ? '先做原始決策' : '現在只改一個語義條件，再做一次'}</h2>
            <p className="mt-2 text-sm leading-6 text-slate-400">答案與改變的維度在兩題都鎖定前不揭露，避免提示污染 transfer 測試。</p>
            {side === 'right' && <div className="mt-4 rounded-xl border border-fuchsia-500/20 bg-fuchsia-500/7 px-4 py-3 text-sm text-fuchsia-200">A 已鎖定。B 和 A 只有一個可觀測語義維度不同，而且兩列都有 PokerBench solver label。</div>}
          </section>
          ...
          <section className="rounded-2xl border border-slate-800 bg-slate-900/55 p-5">
            <div className="text-sm font-semibold">你有多確定？</div>
            <div className="mt-3 grid gap-2 sm:grid-cols-4">
              {CONFIDENCE.map(item => <button key={item.value} onClick={() => setConfidence(item.value)} className={`rounded-xl border px-3 py-3 text-sm ${confidence === item.value ? 'border-amber-400/60 bg-amber-400/10 text-amber-200' : 'border-slate-700 text-slate-400'}`}>{item.label}</button>)}
            </div>
            <button disabled={!choice || !confidence} onClick={lockDecision} className="mt-4 w-full rounded-xl bg-cyan-500 px-5 py-3 font-semibold text-cyan-950 disabled:opacity-40">{side === 'left' ? '鎖定 A，不看答案 →' : '鎖定 B 並揭曉'}</button>
          </section>
        </>
...
export function SemanticCounterfactualTrainer({ onExit }: { onExit: () => void }) {
  ...
  const record = (item: HistoryItem) => setHistory(previous => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-4 py-6 md:px-8">
      <div className="mx-auto mb-5 flex max-w-5xl flex-wrap gap-2">
        <button onClick={() => setSplit('preflop')} className={`rounded-xl border px-4 py-2 text-sm ${split === 'preflop' ? 'border-cyan-400/50 bg-cyan-500/10 text-cyan-200' : 'border-slate-700 text-slate-400'}`}>Preflop</button>
        <button onClick={() => setSplit('postflop')} className={`rounded-xl border px-4 py-2 text-sm ${split === 'postflop' ? 'border-cyan-400/50 bg-cyan-500/10 text-cyan-200' : 'border-slate-700 text-slate-400'}`}>Postflop</button>
        <span className="self-center text-xs text-slate-500">找到 {allPairs.length} 組 one-variable solver flips</span>
      </div>
      <SemanticCounterfactualSession pairs={pairs} history={history} onRecord={record} onExit={onExit} onComplete={onExit} />
    </div>
  );
}
...
function SolverSpot({ row, choice, onChoice }: { row: PokerBenchRow; choice: string | null; onChoice: (choice: string) => void }) {
  ...
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-900/55 p-5 md:p-6">
      <div className="font-mono text-2xl font-bold">{row.holding} · {row.heroPosition}</div>
      <div className="mt-3 grid gap-2 text-sm text-slate-400 sm:grid-cols-3">
        <span>Pot {row.potSize} BB</span>
        <span>{row.split === 'preflop' ? `${row.numPlayers} 人桌 · bet depth ${row.numBets}` : row.evaluationAt}</span>
        <span>{row.split === 'postflop' ? `Board ${board}` : 'Preflop'}</span>
      </div>
      <div className="mt-4 rounded-xl border border-slate-800 bg-slate-950/35 p-4 text-sm leading-6 text-slate-300">
        {row.split === 'preflop' ? row.prevLine || 'First in' : <><div>Preflop: {row.preflopAction || '-'}</div><div>Postflop: {row.postflopAction || '-'}</div></>}
      </div>
      <div className="mt-5 grid gap-2 sm:grid-cols-2">
        {row.availableMoves.map(move => <button key={move} onClick={() => onChoice(move)} className={`rounded-xl border p-3 text-left text-sm ${choice === move ? 'border-cyan-400/60 bg-cyan-500/12 text-cyan-100' : 'border-slate-700 bg-slate-950/35 text-slate-300'}`}>{move}</button>)}
      </div>
    </section>
  );
}
...
function SemanticReveal({ pair, answers, onNext }: { pair: SemanticDecisionPair; answers: PairAnswers; onNext: () => void }) {
  ...
  return (
    <section className="rounded-3xl border border-emerald-500/20 bg-emerald-500/6 p-6 md:p-8">
      <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-300"><GitCompareArrows className="h-4 w-4" />True counterfactual reveal</div>
      <h2 className="mt-3 text-2xl font-bold">答案真的翻了，差的是：{semanticDimensionLabel(pair.dimension)}</h2>
      <p className="mt-3 rounded-xl border border-slate-800 bg-slate-950/35 p-4 text-sm text-slate-300">{describeSemanticChange(pair)}</p>
      <div className="mt-5 grid gap-4 md:grid-cols-2"><RevealCard label="A" choice={left.choice} best={pair.left.correctDecision} /><RevealCard label="B" choice={right.choice} best={pair.right.correctDecision} /></div>
      <div className="mt-5 rounded-xl border border-amber-500/20 bg-amber-500/7 p-4 text-sm leading-6 text-amber-100">這裡只聲稱「這一個可觀測維度改變，而且 pinned PokerBench solver label 隨之改變」。PokerBench 沒有發布每個 action EV、mixed frequency 或因果解釋，所以系統不會捏造原因。</div>
      <button onClick={onNext} className="mt-5 rounded-xl bg-emerald-500 px-5 py-3 font-semibold text-emerald-950">下一組</button>
    </section>
  );
}
