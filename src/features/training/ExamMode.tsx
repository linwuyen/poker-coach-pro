...
export function ExamMode({ onExit }: { onExit: () => void }) {
  ...
  const pool = useMemo<ExamCandidate[]>(() => {
    ...
    const seen = (candidate: ExamCandidate) => candidate.kind === 'scenario'
      ? history.some(item => item.trainingType === 'benchmark' && item.scenarioId === candidate.scenario.id && item.stepId === candidate.scenario.steps[candidate.stepIndex]?.id)
      : history.some(item => item.trainingType === 'solver-benchmark' && item.datasetRowId === candidate.row.id);
  ...
  function answerScenario(action: ActionType) {
    ...
    persist({
      ...
      isUnseen: !history.some(item => item.trainingType === 'benchmark' && item.scenarioId === scenario.id && item.stepId === step.id),
      isTransferTest: true,
  ...
  function answerSolver(move: string) {
    ...
    persist({
      ...
      isReview: false,
      isDelayedReview: false,
      isUnseen: !history.some(item => item.trainingType === 'solver-benchmark' && item.datasetRowId === row.id),
      isTransferTest: true,
      transferLevel: 'structural',
  ...
  if (loadingSolver && !pool.length) return <div className="grid min-h-screen place-items-center bg-slate-950 text-sm text-slate-500">正在建立 hidden exam pool…</div>;
  if (!pool.length) return <EmptyExam onExit={onExit} />;
  if (complete) return <ExamReport items={sessionItems} onExit={onExit} onRestart={() => { setIndex(0); setSessionItems([]); }} />;
  ...
  return <div className="min-h-screen bg-slate-950 px-4 py-6 text-slate-100 md:px-8" data-testid="exam-mode">
    <div className="mx-auto max-w-5xl space-y-5">
      ...
      {candidate?.kind === 'scenario' ? <ScenarioExamCard candidate={candidate} onAnswer={answerScenario} /> : candidate ? <SolverExamCard row={candidate.row} onAnswer={answerSolver} /> : null}
    </div>
  </div>;
}
...
function ScenarioExamCard({ candidate, onAnswer }: { candidate: Extract<ExamCandidate, { kind: 'scenario' }>; onAnswer: (action: ActionType) => void }) {
  ...
  return <section className="rounded-3xl border border-slate-800 bg-slate-900/55 p-6 md:p-8"><div className="text-xs text-slate-500">{scenario.type} · {scenario.position} · {step.street} · Effective {scenario.effectiveStack}</div><h2 className="mt-2 text-xl font-bold">{scenario.title}</h2><div className="mt-5 rounded-2xl border border-emerald-500/15 bg-slate-950/45 p-6 text-center"><div className="flex min-h-20 items-center justify-center gap-2">{step.communityCards.length ? step.communityCards.map((card, i) => <CardUI key={`${card.rank}-${card.suit}-${i}`} card={card} size="sm" />) : <span className="text-xs text-slate-600">Preflop</span>}</div><div className="mt-4 font-mono text-sm text-emerald-300">Pot {step.potSize} BB</div><div className="mt-5 flex justify-center gap-2">{scenario.holeCards.map((card, i) => <CardUI key={`${card.rank}-${card.suit}-${i}`} card={card} size="sm" />)}</div></div><div className="mt-4 grid gap-3 md:grid-cols-2"><Context label="前序行動" value={scenario.preAction} /><Context label="現在的問題" value={step.description} /></div><div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{step.options.map(action => <button data-testid="exam-action" key={action} type="button" onClick={() => onAnswer(action)} className="rounded-xl border border-slate-700 bg-slate-950/35 p-4 text-left text-sm font-semibold hover:border-amber-500/40">{ACTION_LABELS[action] || action}</button>)}</div></section>;
}
