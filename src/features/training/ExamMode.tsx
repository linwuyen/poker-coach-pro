...
const ACTION_LABELS: Partial<Record<ActionType, string>> = {
  Fold: '棄牌', Call: '跟注', Raise: '加注', '3-bet': '3-Bet', '4-bet (Raise)': '4-Bet', 'All-in': '全下',
  Check: '過牌', 'Bet small': '小注', 'Bet half pot': '半池', 'Bet big': '大注',
};
...
export function ExamMode({ onExit }: { onExit: () => void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([loadPokerBenchSplit('preflop'), loadPokerBenchSplit('postflop')])
      ...
      .catch(() => { if (!cancelled) setLoadingSolver(false); });
  ...
  const pool = useMemo<ExamCandidate[]>(() => {
    ...
    const sort = (items: ExamCandidate[]) => [...items].sort((left, right) => Number(seen(left)) - Number(seen(right)) || stableHash(left.id) - stableHash(right.id));
  ...
  function persist(item: HistoryItem) {
    ...
    setHistory(previous => { const next = [...previous, item]; saveHistory(next); return next; });
  ...
  function answerScenario(action: ActionType) {
    ...
    persist({
      schemaVersion: 6, attemptId: createAttemptId(), trainingType: 'benchmark', scenarioId: scenario.id,
      decisionFamilyId: scenario.decisionFamilyId || scenario.id, stepId: step.id,
      ...
      score: feedback.score, judgment: feedback.judgment, timestamp: now, selectedAction: action, bestAction: feedback.bestAction,
      street: step.street, position: scenario.position, durationMs: now - startedAt.current, correct,
      feedbackQuality: correct ? 'best' : 'major-error', chosenEvBB: feedback.evidence?.actionEvBB,
      bestEvBB: feedback.evidence?.bestEvBB, evLossBB: feedback.evidence?.evLossBB,
      truthTier: feedback.evidence?.sourceConfidence || 'expert-baseline', difficultyWeight: getDifficultyWeight(scenario.difficulty),
      isReview: false, isDelayedReview: false,
      isUnseen: !initialHistory.some(item => item.trainingType === 'benchmark' && item.scenarioId === scenario.id && item.stepId === step.id),
      isTransferTest: true, transferLevel: 'structural', questionLabel: scenario.title,
      gameFormat: scenario.type === 'Tournament' ? 'MTT' : 'Cash', contextFamilyId: scenarioContextFamilyId(scenario),
      utilityUnit: hasVerifiedCashEv ? 'bb' : undefined, utilityModel: hasVerifiedCashEv ? 'cash-chip-ev' : undefined,
      examSessionId, examMode: true,
  ...
  function answerSolver(move: string) {
    ...
    persist({
      schemaVersion: 6, attemptId: createAttemptId(), trainingType: 'solver-benchmark',
      scenarioId: `exam-solver:${row.split}:${row.id}`, decisionFamilyId: family, stepId: 'solver-decision',
      ...
      situationIds: [`situation.position.${row.heroPosition.toLowerCase()}`, row.split === 'preflop' ? 'situation.street.preflop' : `situation.street.${row.evaluationAt.toLowerCase()}`],
      category: ['PokerBench', 'Hidden Exam', row.split === 'preflop' ? 'Preflop' : row.evaluationAt],
      score: correct ? 10 : 0, judgment: correct ? '正確' : '錯誤', timestamp: now,
      selectedAction: move, bestAction: row.correctDecision, selectedDecision: selected.action, bestDecision: best.action,
      street: row.split === 'preflop' ? 'Preflop' : row.evaluationAt, position: row.heroPosition,
      durationMs: now - startedAt.current, correct, feedbackQuality: correct ? 'best' : 'major-error',
      truthTier: 'verified-solver', truthSourceId: POKERBENCH_SOURCE.id, truthSourceRef: POKERBENCH_SOURCE.dataset,
      truthSourceLicense: POKERBENCH_SOURCE.license, truthSourceRevision: POKERBENCH_SOURCE.revision,
      datasetSplit: POKERBENCH_FILES[row.split].split, datasetRowId: row.id,
      isReview: false, isDelayedReview: false,
      isUnseen: !initialHistory.some(item => item.trainingType === 'solver-benchmark' && item.datasetRowId === row.id),
      isTransferTest: true, transferLevel: 'structural', solverCorpusRole: 'holdout', gameFormat: 'Cash',
      examSessionId, examMode: true, questionLabel: `Hidden Solver · ${row.holding}`,
  ...
  if (complete) return <ExamReport items={sessionItems} onExit={onExit} onRestart={() => { submissionLock.current = false; setSubmitting(false); setIndex(0); setSessionItems([]); }} />;
  ...
  return <div className="min-h-screen bg-slate-950 px-4 py-6 text-slate-100 md:px-8" data-testid="exam-mode">
    <div className="mx-auto max-w-5xl space-y-5">
      <header className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
        ...
        <div className="min-w-[220px] flex-1"><div className="flex justify-between text-sm"><b>Hidden Exam · 不給即時答案</b><span className="font-mono text-slate-500">{index + 1}/{pool.length}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-800"><div className="h-full bg-amber-400" style={{ width: `${progress}%` }} /></div></div></div>
...
function ScenarioExamCard({ candidate, disabled, onAnswer }: { candidate: Extract<ExamCandidate, { kind: 'scenario' }>; disabled: boolean; onAnswer: (action: ActionType) => void }) {
  const { scenario, stepIndex } = candidate; const step = scenario.steps[stepIndex];
  return <section className="rounded-3xl border border-slate-800 bg-slate-900/55 p-6 md:p-8"><div className="text-xs text-slate-500">{scenario.type} · {scenario.position} · {step.street} · Effective {scenario.effectiveStack}</div><h2 className="mt-2 text-xl font-bold">{scenario.title}</h2><div className="mt-5 rounded-2xl border border-emerald-500/15 bg-slate-950/45 p-6 text-center"><div className="flex min-h-20 items-center justify-center gap-2">{step.communityCards.length ? step.communityCards.map((card, index) => <CardUI key={`${card.rank}-${card.suit}-${index}`} card={card} size="sm" />) : <span className="text-xs text-slate-600">Preflop</span>}</div><div className="mt-4 font-mono text-sm text-emerald-300">Pot {step.potSize} BB</div><div className="mt-5 flex justify-center gap-2">{scenario.holeCards.map((card, index) => <CardUI key={`${card.rank}-${card.suit}-${index}`} card={card} size="sm" />)}</div></div><div className="mt-4 grid gap-3 md:grid-cols-2"><Context label="前序行動" value={scenario.preAction} /><Context label="現在的問題" value={step.description} /></div><div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{step.options.map(action => <button data-testid="exam-action" key={action} type="button" disabled={disabled} onClick={() => onAnswer(action)} className="rounded-xl border border-slate-700 bg-slate-950/35 p-4 text-left text-sm font-semibold hover:border-amber-500/40 disabled:cursor-not-allowed disabled:opacity-40">{ACTION_LABELS[action] || action}</button>)}</div></section>;
}

function SolverExamCard({ row, disabled, onAnswer }: { row: PokerBenchRow; disabled: boolean; onAnswer: (move: string) => void }) {
  const hero = parseSolverCards(row.holding); const board = row.split === 'postflop' ? parseSolverCards(`${row.boardFlop}${row.boardTurn || ''}${row.boardRiver || ''}`) : [];
  const preflop = row.split === 'preflop' ? humanizeSolverPreflopLine(row.prevLine) : humanizeSolverPreflopLine(row.preflopAction); const postflop = row.split === 'postflop' ? humanizeSolverPostflopLine(row.postflopAction) : '';
  return <section className="rounded-3xl border border-slate-800 bg-slate-900/55 p-6 md:p-8"><div className="text-xs text-slate-500">PokerBench Holdout · {row.heroPosition} · {row.split === 'preflop' ? 'Preflop' : row.evaluationAt}</div><div className="mt-5 rounded-2xl border border-emerald-500/15 bg-slate-950/45 p-6 text-center"><div className="flex min-h-20 items-center justify-center gap-2">{board.length ? board.map((card, index) => <CardUI key={`${card.rank}-${card.suit}-${index}`} card={card} size="sm" />) : <span className="text-xs text-slate-600">Preflop</span>}</div><div className="mt-4 font-mono text-sm text-emerald-300">Pot {row.potSize} BB</div><div className="mt-5 flex justify-center gap-2">{hero.map((card, index) => <CardUI key={`${card.rank}-${card.suit}-${index}`} card={card} size="sm" />)}</div></div><div className="mt-4 rounded-xl border border-slate-800 bg-slate-950/35 p-4 text-sm leading-7 text-slate-300"><div><span className="text-slate-500">翻前：</span>{preflop}</div>{postflop && <div><span className="text-slate-500">翻後：</span>{postflop}</div>}</div><div className="mt-5 grid gap-2 sm:grid-cols-2">{row.availableMoves.map(move => <button data-testid="exam-action" key={move} type="button" disabled={disabled} onClick={() => onAnswer(move)} className="rounded-xl border border-slate-700 bg-slate-950/35 p-4 text-left text-sm font-semibold hover:border-amber-500/40 disabled:cursor-not-allowed disabled:opacity-40">{humanizeSolverMove(move)}</button>)}</div></section>;
}

function ExamReport({ items, onExit, onRestart }: { items: HistoryItem[]; onExit: () => void; onRestart: () => void }) {
  const correctCount = items.filter(item => item.correct).length; const accuracy = items.length ? Math.round(correctCount / items.length * 100) : 0;
