  ...
  if (!row) {
    return (
      <div className="mx-auto max-w-3xl rounded-3xl border border-emerald-500/20 bg-emerald-500/6 p-8 text-center text-slate-100">
        ...
        <h2 className="mt-4 text-2xl font-bold">Solver 泛化完成</h2>
        <p className="mt-2 text-sm text-slate-400">完成 {rows.length} 個 training-partition solver decisions。</p>
        <div className="mt-6 flex justify-center gap-3">
          <button onClick={onComplete} className="rounded-xl bg-emerald-500 px-5 py-3 font-semibold text-emerald-950">完成今日訓練</button>
    ...
    <div className="mx-auto max-w-5xl space-y-5 text-slate-100">
      <header className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
        <div className="flex items-center gap-3">
          <button onClick={onExit} className="flex items-center gap-2 rounded-lg px-2 py-2 text-sm text-slate-400 hover:bg-slate-800"><ArrowLeft className="h-4 w-4" />離開</button>
          <div className="flex-1">
            <div className="flex justify-between text-sm"><b>{title}</b><span className="font-mono text-slate-500">{index + 1}/{rows.length}</span></div>
            <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-800"><div className="h-full bg-blue-400" style={{ width: `${progress}%` }} /></div>
          </div>
        </div>
      </header>

      <section className="rounded-3xl border border-blue-500/20 bg-[linear-gradient(135deg,rgba(59,130,246,0.12),rgba(15,23,42,0.82))] p-6 md:p-8">
        <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-blue-300"><ShieldCheck className="h-4 w-4" />verified-solver · training partition</div>
        <h2 className="mt-3 font-mono text-3xl font-bold">{row.holding} · {row.heroPosition}</h2>
        <div className="mt-4 grid gap-2 text-sm text-slate-400 sm:grid-cols-3">
          <span>Pot {row.potSize} BB</span>
          <span>{row.split === 'preflop' ? `Players ${row.numPlayers}` : row.evaluationAt}</span>
          <span>{row.split === 'postflop' ? `Board ${board}` : `Bet depth ${row.numBets}`}</span>
        </div>
        ...
        <div className="mt-5 grid gap-2 sm:grid-cols-2">
          {row.availableMoves.map(move => <button key={move} disabled={submitted} onClick={() => setChoice(move)} className={`rounded-xl border p-3 text-left text-sm ${choice === move ? 'border-blue-400/60 bg-blue-500/12' : 'border-slate-700 bg-slate-950/35'}`}>{move}</button>)}
        </div>
      </section>
      ...
      {!submitted ? (
        <section className="rounded-2xl border border-slate-800 bg-slate-900/55 p-5">
          ...
          <button disabled={!choice || !confidence} onClick={submit} className="mt-4 w-full rounded-xl bg-blue-500 px-5 py-3 font-semibold text-white disabled:opacity-40">提交</button>
        </section>
      ) : (
        <section className="rounded-2xl border border-emerald-500/20 bg-emerald-500/6 p-5">
          <div className="text-sm">你的答案：<b>{choice}</b></div>
          <div className="mt-1 text-sm">Solver label：<b>{row.correctDecision}</b></div>
          <p className="mt-3 text-xs leading-6 text-slate-400">PokerBench 未提供此列的 per-action EV 或 mixed frequency，因此只評 optimal decision label，不補假數字。</p>
          <button onClick={next} className="mt-4 rounded-xl bg-emerald-500 px-5 py-3 font-semibold text-emerald-950">下一題</button>
        </section>
      )}
    </div>
  );
