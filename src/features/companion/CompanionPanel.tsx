      ...
      {!state ? <EmptyCompanion /> : <>
        ...
        <section className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <section className="rounded-2xl border border-slate-800 bg-slate-900/55 p-5">
            <div className="flex items-center gap-2 text-sm font-semibold"><Sparkles className="h-4 w-4 text-emerald-400" />這手下一個訓練動作</div>
            {analysis.intervention ? <><div className="mt-4 text-xl font-bold text-white">{analysis.intervention.label}</div><p className="mt-2 text-xs leading-5 text-slate-400">{analysis.intervention.reason}</p>{analysis.intervention.hash && <button type="button" disabled={!analysis.policy.canOpenDecisionTools} onClick={() => openTool(analysis.intervention?.hash)} className="mt-5 flex items-center gap-2 rounded-xl bg-emerald-500 px-4 py-2.5 text-xs font-bold text-emerald-950 disabled:cursor-not-allowed disabled:opacity-30">開啟分析<ExternalLink className="h-3.5 w-3.5" /></button>}</> : <p className="mt-4 text-xs leading-5 text-slate-500">先完成目前訓練決策；作答後才會推薦最值得深挖的工具。</p>}
          </section>
        </section>

        <section className="rounded-2xl border border-slate-800 bg-slate-900/55 p-5">
          <div className="flex items-center justify-between"><div><h2 className="text-sm font-semibold">作答後深挖</h2><p className="mt-1 text-xs text-slate-500">工具只服務目前這手的學習目的，不再當成獨立產品入口。</p></div><Target className="h-5 w-5 text-slate-600" /></div>
