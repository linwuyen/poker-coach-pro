...
export function CompanionPanel({ onExit }: CompanionPanelProps) {
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-100">
    ...
    <main className="mx-auto max-w-5xl space-y-4 p-4 md:p-6">
      {!state ? <EmptyCompanion /> : <>
        ...
        <section className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <section className="rounded-2xl border border-slate-800 bg-slate-900/55 p-5">
            <div className="flex items-center gap-2 text-sm font-semibold"><Sparkles className="h-4 w-4 text-emerald-400" />推薦 Intervention</div>
            {analysis.intervention ? <><div className="mt-4 text-xl font-bold text-white">{analysis.intervention.label}</div><p className="mt-2 text-xs leading-5 text-slate-400">{analysis.intervention.reason}</p>{analysis.intervention.hash && <button type="button" disabled={!analysis.policy.canOpenDecisionTools} onClick={() => openTool(analysis.intervention?.hash)} className="mt-5 flex items-center gap-2 rounded-xl bg-emerald-500 px-4 py-2.5 text-xs font-bold text-emerald-950 disabled:cursor-not-allowed disabled:opacity-30">開啟工具<ExternalLink className="h-3.5 w-3.5" /></button>}</> : <p className="mt-4 text-xs leading-5 text-slate-500">目前 gate 不允許顯示決策介入；牌局完成後會自動解鎖。</p>}
          </section>
        </section>

        <section className="rounded-2xl border border-slate-800 bg-slate-900/55 p-5">
          <div className="flex items-center justify-between"><div><h2 className="text-sm font-semibold">工具箱已變成 Companion instruments</h2><p className="mt-1 text-xs text-slate-500">預設只突出推薦工具；需要時才打開其他診斷工具。</p></div><Target className="h-5 w-5 text-slate-600" /></div>
      ...
      <ManualBridge value={manual} onChange={setManual} onSync={syncManual} current={state} />
    </main>
  </div>;
}
...
function StrategyCard({ state, analysis }: { state: CompanionHandState; analysis: ReturnType<typeof analyzeCompanionState> }) {
  if (!analysis.policy.canShowStrategy) return <section className="rounded-2xl border border-slate-800 bg-slate-900/55 p-5"><div className="flex items-center gap-2 text-sm font-semibold"><LockKeyhole className="h-4 w-4 text-amber-300" />策略輸出已鎖定</div><p className="mt-3 text-xs leading-5 text-slate-500">保留目前 context，但不顯示 Fold / Call / Raise / Jam 頻率與 EV。</p></section>;
  if (state.street !== 'Preflop') return <section className="rounded-2xl border border-slate-800 bg-slate-900/55 p-5"><div className="flex items-center gap-2 text-sm font-semibold"><Calculator className="h-4 w-4 text-blue-300" />翻後 Companion</div><p className="mt-3 text-xs leading-5 text-slate-500">Preflop Strategy Engine 不冒充翻後 solver。此 street 先用 SPR / Pot Odds 與推薦 intervention，完整翻後策略交給 Solver / Boundary 工具。</p></section>;
