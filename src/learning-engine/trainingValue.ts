...
import { historyDecisionFamilyId, scenarioDecisionFamilyId } from './contextIdentity';
import { matchingUtilityEvidence, scenarioUtilityMode } from './utility';

export type EvGainEvidence = 'verified' | 'observed' | 'estimated';
export type SpotFrequencySource = 'scenario-prior' | 'heuristic-prior';
...
/** Scenario frequency is a curriculum prior used for ranking, never a claim about this player's real table frequency. */
...
export function expectedLearningValue(
  ...
  const measuredUtilityRegret = average(utilityEvidence.map(entry => entry.observation.loss));
  ...
  const regretForRanking = measuredUtilityRegret ?? fallbackRegret;
  const observedEvRegretBB = scenario.type === 'Cash Game' && measuredUtilityRegret !== undefined ? measuredUtilityRegret : 0;

  const spotFrequencyPer100Hands = estimateSpotFrequencyPer100Hands(scenario);
  const spotFrequencySource: SpotFrequencySource = typeof scenario.spotFrequencyPer100Hands === 'number' && scenario.spotFrequencyPer100Hands > 0
    ? 'scenario-prior'
    : 'heuristic-prior';
  ...
  const evGainEvidence: EvGainEvidence = hasVerifiedUtility ? 'verified' : hasObservedUtility ? 'observed' : 'estimated';
  ...
  // Frequency is a curriculum prior, not observed real-play exposure. Keep numeric gain internal for ranking;
  // do not surface it as a bankroll/win-rate report.
  const reportableExpectedUtilityGainPer100Hands = undefined;
  const reportableExpectedEvGainPer100Hands = undefined;
