...
export interface MultiwayTruthStore {
  ...
  putManifest(manifest:MultiwayTruthPackManifest):Promise<void>;
...
class MemoryStore implements MultiwayTruthStore{
  ...
  async putManifest(manifest:MultiwayTruthPackManifest){this.manifests.set(manifest.key,{...manifest});}
...
class IndexedStore implements MultiwayTruthStore{
  ...
  async putManifest(manifest:MultiwayTruthPackManifest){const db=await openDb();try{const tx=db.transaction(PACK_STORE,'readwrite');tx.objectStore(PACK_STORE).put({...manifest});await done(tx);}finally{db.close();}}
...
export async function importMultiwayTruthNdjson(lines:AsyncIterable<string>,store:MultiwayTruthStore,meta:{packId:string;version:string;sourceReference:string}){let imported=0,skipped=0,contentBytes=0;for await(const raw of lines){const line=raw.trim();if(!line||line.startsWith('#'))continue;contentBytes+=new TextEncoder().encode(line).byteLength;const result=await store.putNodes([JSON.parse(line) as MultiwayTruthNodeV4]);imported+=result.imported;skipped+=result.skipped;}await store.putManifest({key:`${meta.packId}@${meta.version}`,packId:meta.packId,version:meta.version,importedAt:new Date().toISOString(),sourceReference:meta.sourceReference,nodeCount:imported,skippedCount:skipped,contentBytes});return{imported,skipped};}
