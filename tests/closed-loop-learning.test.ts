...
import { adaptiveCalibrationReport, buildKnowledgeStates, candidateLearningSignal, isVerifiedEvaluationEv, verifiedEvNorthStar } from '../src/learning-engine/closedLoop';
...
import { buildExactMathEvaluationScenarios, exactMathEvaluationScenarios } from '../src/teaching/evaluationMathScenarios';
...
test('isolated exact-math evaluation seed bank can produce verified cash-BB EV samples',()=>{
  assert.equal(exactMathEvaluationScenarios.length,16);
  const catalogIds=new Set(scenarioCatalog.map(scenario=>scenario.id));
  assert.ok(exactMathEvaluationScenarios.every(scenario=>scenario.benchmarkRole==='holdout'));
  assert.ok(exactMathEvaluationScenarios.every(scenario=>!catalogIds.has(scenario.id)));
  const scenario=exactMathEvaluationScenarios[0];
  const step=scenario.steps[0];
  const feedback=Object.values(step.feedbacks).find(candidate=>typeof candidate?.evidence?.evLossBB==='number'&&(candidate.evidence.evLossBB||0)>0);
  assert.ok(feedback?.evidence);
  const item=historyItem({trainingType:'benchmark',scenarioId:scenario.id,truthTier:feedback!.evidence!.sourceConfidence,gameFormat:'Cash',utilityUnit:'bb',utilityModel:'cash-chip-ev',evLossBB:feedback!.evidence!.evLossBB,isTransferTest:true,examMode:true});
  assert.equal(isVerifiedEvaluationEv(item),true);
});

test('exact-math evaluation generator replenishes fresh holdout variants beyond the seed bank',()=>{
  const catalogIds=new Set(scenarioCatalog.map(scenario=>scenario.id));
  const seedIds=new Set(exactMathEvaluationScenarios.map(scenario=>scenario.id));
  const replenished=buildExactMathEvaluationScenarios(64000,50);
  assert.equal(replenished.length,50);
  assert.equal(new Set(replenished.map(scenario=>scenario.id)).size,50);
  assert.ok(replenished.every(scenario=>scenario.benchmarkRole==='holdout'));
  assert.ok(replenished.every(scenario=>!seedIds.has(scenario.id)));
  assert.ok(replenished.every(scenario=>!catalogIds.has(scenario.id)));
  const scenario=replenished[17];
  const feedback=Object.values(scenario.steps[0].feedbacks).find(candidate=>typeof candidate?.evidence?.evLossBB==='number'&&(candidate.evidence.evLossBB||0)>0);
  assert.ok(feedback?.evidence);
  assert.equal(feedback!.evidence!.sourceConfidence,'exact-math');
  assert.equal(Number.isFinite(feedback!.evidence!.evLossBB),true);
  assert.equal(isVerifiedEvaluationEv(historyItem({trainingType:'benchmark',scenarioId:scenario.id,truthTier:'exact-math',gameFormat:'Cash',utilityUnit:'bb',utilityModel:'cash-chip-ev',evLossBB:feedback!.evidence!.evLossBB,isTransferTest:true,examMode:true,isUnseen:true})),true);
});
...
test('current-step situation inference exposes only the street actually tested',()=>{
  const scenario={id:'multi-street-test',title:'multi',category:[],difficulty:'中階',type:'Cash Game',blinds:'1/2',ante:false,userStack:'100 BB',userBB:100,position:'BTN',holeCards:[],preAction:'x',effectiveStack:'100 BB',tableSize:'6max',steps:[{id:'flop',street:'Flop',communityCards:[],description:'f',potSize:10,options:[],feedbacks:{}},{id:'turn',street:'Turn',communityCards:[],description:'t',potSize:20,options:[],feedbacks:{}},{id:'river',street:'River',communityCards:[],description:'r',potSize:30,options:[],feedbacks:{}}]} as Scenario;
...
test('solver decisions are first-class knowledge-state evidence',()=>{
  const states=buildKnowledgeStates([historyItem({trainingType:'solver-corpus',skillIds:['preflop.solver-decision'],category:['PokerBench','Preflop'],street:'Preflop',truthTier:'verified-solver',correct:true})]);
...
test('active learning favors uncertainty and due repair instead of only recent novelty',()=>{
  ...
  const candidate={kind:'scenario',id:'curated:test',source:'curated',familyId:scenario.decisionFamilyId||scenario.id,presentationFingerprint:'x',truthLabel:'exact',street:scenario.steps[0].street,position:scenario.position,format:'cash',stackBand:'80-125',actionClass:'call',scenario} as unknown as InfiniteHandCandidate;
...
test('closed-loop product surfaces are actually wired into the player flow',()=>{
  ...
  assert.match(training,/skillIds\s*:\s*inferScenarioStepSkillIds\(scenario,\s*step\)/);
  assert.match(training,/situationIds\s*:\s*inferSituationIdsFromScenarioStep\(scenario,\s*step\)/);
  assert.match(training,/trainingDwellMs\s*:\s*Math\.max\(0,\s*Date\.now\(\)\s*-\s*startedAt\.current\)/);
  assert.match(training,/utilityModel\s*:\s*verifiedCashEv\s*\?\s*'cash-chip-ev'/);
  assert.match(solver,/trainingDwellMs\s*:\s*Math\.max\(0,\s*Date\.now\(\)\s*-\s*startedAt\.current\)/);
  assert.match(exam,/buildExactMathEvaluationScenarios/);
  ...
  assert.match(exam,/skillIds\s*:\s*inferScenarioStepSkillIds\(scenario,\s*step\)/);
  assert.match(exam,/situationIds\s*:\s*inferSituationIdsFromScenarioStep\(scenario,\s*step\)/);
